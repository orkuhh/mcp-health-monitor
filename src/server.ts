...
const server = new Server(
  { name: "mcp-health-monitor", version: "1.0.0" },
  { capabilities: { tools: {} } }
);

// Cache for server statuses
let serverStatuses: Map<string, MCPServerStatus> = new Map();
let lastHealthCheck: Date | null = null;

// Health check tracking
const healthCheckCache: Map<string, { healthy: boolean; timestamp: Date }> = new Map();
...
async function loadMCPServerConfigs(): Promise<Map<string, MCPServerConfig>> {
  try {
...
async function getProcessInfo(pid: number): Promise<{ exists: boolean; uptime?: number }> {
  try {
...
async function checkServerHealth(name: string, config: MCPServerConfig): Promise<MCPServerStatus> {
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `ps aux | grep -E "${config.command}|${config.args.join(' ')}" | grep -v grep | awk '{print $2}' | head -1`
    );
...
async function pingServer(name: string): Promise<boolean> {
...
async function restartServer(name: string, config: MCPServerConfig): Promise<{ success: boolean; message: string }> {
  try {
    // Kill existing process if running
    if (config.args.length > 0) {
      try {
        const { stdout } = await execAsync(
          `ps aux | grep "${config.args.join(' ')}" | grep -v grep | awk '{print $2}' | xargs -r kill 2>/dev/null || true`
        );
      } catch {
...
async function listServers(): Promise<{ servers: MCPServerStatus[] }> {
...
async function checkHealth(name: string): Promise<MCPServerStatus | null> {
  const configs = await loadMCPServerConfigs();
...
async function checkAllHealth(): Promise<{ servers: MCPServerStatus[]; checkedAt: string }> {
  const result = await listServers();
  return { ...result, checkedAt: new Date().toISOString() };
}

async function restartMCPServer(name: string): Promise<{ success: boolean; message: string }> {
  const configs = await loadMCPServerConfigs();
  const config = configs.get(name);
...
async function getUnhealthyServers(): Promise<{ servers: MCPServerStatus[] }> {
  const result = await listServers();
  return {
...
async function restartAllUnhealthy(): Promise<{ results: { name: string; success: boolean; message: string }[] }> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "check_health":
        const serverName = (args as any)?.name;
        if (!serverName) {
      ...
      case "restart_server":
        const restartName = (args as any)?.name;
        if (!restartName) {
          throw new Error("Server name is required");
        }
